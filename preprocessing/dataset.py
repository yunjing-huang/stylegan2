...
def crop_to_rectangular(image):
    ...
    if delta > 0:
        arr = np.zeros((delta,image.shape[1],image.shape[2]))
        cropped_img = np.concatenate([arr[:delta // 2,:,:],image,arr[delta // 2:,:,:]],axis=0)
    elif delta < 0:
        arr = np.zeros((image.shape[0], -delta, image.shape[2]))
        cropped_img = np.concatenate([arr[:,:-delta // 2, :], image, arr[:,delta // 2:, :]], axis=1)
    else:
        cropped_img = image
...
def preprocess_images(input_dir, out_dir, resolution, to_grayscale=False):
    ...
    for image_file_name in image_file_names:
        ...
        croped_image = tf.image.resize_with_pad(image_array,resolution, resolution)
        final_img = Image.fromarray(croped_image.numpy().astype('uint8'))
...
if __name__ == "__main__":
    preprocess_data('C:/Users/Yunji/PycharmProjects/python2/config/flowers.yaml', 'flowers', 'C:/Users/Yunji/PycharmProjects/python2/data/flowers', 'C:/Users/Yunji/PycharmProjects/python2/outdata')
